# Infrastructure Examples — fusionAIze Grid

The **4+1 Node Architecture** of fusionAIze Grid intentionally decouples the *functional role* of a node from its *physical hardware*. 
...
2. **grid-core**: A powerful **Mini-PC** (e.g., Intel NUC, 32GB RAM).
...
3. **grid-worker**: An **older MacBook Pro** (e.g., M1 Max with unified memory).
...
### Node Mapping
1. **grid-edge**: A massive **Cloudflare Tunnel / AWS API Gateway** or a dedicated **Nginx VPS**.
   - Handles DDoS protection, global TLS routing, and SSO (Authelia/Authentik) blocking malicious traffic at the perimeter.
2. **grid-core**: A dedicated **Hetzner / AWS EC2 Compute Server** (e.g., 64GB RAM, NVMe).
