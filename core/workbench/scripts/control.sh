...
cmd_configure() {
  ...
  while read -r p; do
    ...
    if [[ "$stat" == *"Not installed"* || "$stat" == "Error" ]]; then
      printf "  ${C_BOLD}%3d)${C_RESET}  %-12s  %-16s  ${C_YELLOW}%-22s${C_RESET}  %b\n" \
        "$i" "$cat" "$name" "$stat" "$cfg_mark"
    else
      printf "  ${C_BOLD}%3d)${C_RESET}  %-12s  %-16s  ${C_GREEN}%-22s${C_RESET}  %b\n" \
        "$i" "$cat" "$name" "$stat" "$cfg_mark"
    fi
    plugin_list+=("$p")
    i=$((i + 1))
  ...
  local total=${#plugin_list[@]}
  echo ""
  printf "  ${C_DIM}✎ = has interactive configuration${C_RESET}\n\n"
  printf "  ${C_BOLD}▸${C_RESET}  Select tool to configure (or q to cancel)\n\n"
  ...
  if [[ "$choice" -ge 1 && "$choice" -le "$total" ]] 2>/dev/null; then
    ...
    divider
    echo ""
    # Source _lib.sh so plugins can use nexus_write_env, nexus_read_env etc.
    (
      source "${HERE}/_lib.sh"
      source "$p"
...
show_menu() {
  while true; do
    ...
    case "$opt" in
      ...
      *) warn "Invalid option — enter 1-5 or q." ;;
    esac
  done
}
